...
public class GrimPanFXController extends AnchorPane {
	...
	public GrimPanFXController() {
		...
		widthProp.addListener((obs, oldVal, newVal) -> {
			...
			lblWinW.setText("Windowsize: "+Math.round(val));
		...
		heightProp.addListener((obs, oldVal, newVal) -> {
			...
			lblWinH.setText("x "+ Math.round(val));
    ...
    @FXML
    private Label lblWinW;
    
    @FXML
    private Label lblWinH;
    ...
    //observer pattern을 사용해서 3개의 변화하는 상태를 기록
